...
rewrite_prompt = ChatPromptTemplate.from_template(
    """Provide the better search query to answer the given question: 
    Question: {query}
    Answer: """
)

def parse_rewriter_result(message):
    return message.content.strip('"')


rewrite_chain = rewrite_prompt | llm | parse_rewriter_result
...
def rrr_rag_pipeline(query):
    ...
    docs = retriever.invoke(new_query) # Fetches the relevant documents
    llm_chain = prompt | llm # generates the answer
    user_input = {"context": docs,
              "question":new_query }
    result = llm_chain.invoke(user_input)
